...
if [ "$#" -lt 3 ]; then
  ...
  echo 'example: `./figure9a.sh perf noop sync`'
  exit 1
fi


function measure_perf {
  CON=(1 3 5 7 10 15 20 30 40 50 70 90 110 130 150 170 200)
...
function run_wrk {
  sleep 10
  IP=$(kubectl get svc ingress-nginx-external -n ingress-nginx -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
  PORT=$(kubectl get svc ingress-nginx-external -n ingress-nginx -o jsonpath='{.spec.ports[0].nodePort}')
  #PORT=$(kubectl get svc router -n fission -o jsonpath='{.spec.ports[0].nodePort}')
  ENTRY_HOST=http://$IP:$PORT
