...
function build_fission_container {
    ...
    sudo docker build --no-cache -t $USERNAME/mm-$FUNC:latest \
...
function build_fission_bin {
    ...
    rm -rf $ROOT_DIR/template/rust/DbInterface
    sudo docker system prune -f
    sudo docker create --name temp-container $USERNAME/mm-$FUNC:latest
...
function push {
  sudo docker push $USERNAME/mm-$FUNC:latest
}
...
function deploy_openwhisk {
  wsk action create $FUNC --docker $USERNAME/mm-$FUNC:latest    
}
...
function deploy_fission_c {
  ...
  fission function run-container --name $FUNC \
    --image docker.io/$USERNAME/mm-$FUNC --port 8888 \
