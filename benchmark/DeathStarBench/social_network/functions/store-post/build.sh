...
function build_fission_container {
    ...
    sudo docker build --no-cache -t $USERNAME/sn-$FUNC:latest \
...
function build_fission_bin {
    ...
    rm -rf $ROOT_DIR/template/rust/DbInterface
    sudo docker system prune -f
    sudo docker create --name temp-container $USERNAME/sn-$FUNC:latest
...
function push {
  sudo docker push $USERNAME/sn-$FUNC:latest
}
...
function deploy_fission_c {
  ...
  fission function run-container --name $FUNC \
    --image docker.io/$USERNAME/sn-$FUNC --port 8888 \
