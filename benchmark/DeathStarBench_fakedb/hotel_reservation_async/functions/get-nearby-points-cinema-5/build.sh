...
function build_fission_container {
    ...
    sudo docker build --no-cache -t $USERNAME/hr-$FUNC-async:latest \
...
function build_fission_bin {
    ...
    rm -rf $ROOT_DIR/template/rust/DbInterface
    sudo docker system prune -f
    sudo docker create --name temp-container $USERNAME/hr-$FUNC-async:latest
...
function push {
  sudo docker push $USERNAME/hr-$FUNC-async:latest
}
...
function deploy_fission_c {
  ...
  fission function run-container --name $FUNC \
    --image docker.io/$USERNAME/hr-$FUNC-async --port 8888 \
